...
public class ProjetService {
    ...
    public List<ProjetDto> findAll() {
        return projetMapper.toDtos(projetRepository.findAllWithAssociations());
    }

    public List<ProjetDto> findAllWithPagination(Pageable pageable) {
        Page<Projet> projetsPage = projetRepository.findAllWithAssociationsPagination(pageable);
        return projetsPage.map(projetMapper::toDto).getContent();
    }

    public ProjetDto findById(long id) {
        Projet projet = projetRepository.findByIdWithAssociations(id)
